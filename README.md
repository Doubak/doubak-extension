...
> 长文正文、导出与导入都在里面。还没做的是相册、豆列与长广播的全文——见下方「现状」。
...
| 移动应用（app）的标记列表 | 豆瓣没有可用的列表入口，见 DESIGN 的「已知无入口」表。`UNRESOLVED_ROUTES` 里现在只剩这一条 |
...
测试：`npm test`（零安装即可跑，69 个测试文件、1572 个测试）。装了可选开发依赖后会额外用
