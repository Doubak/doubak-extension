...
- **上架地址：slug 定为 `doubak`**（2026-09-10 定的），**2026-09-13 上架**：
  ...
  **判据不是页面的 404**，那分不出「不存在」与「不公开」。是 AMO 的公开接口，
  ...
  实测：这个地址 301 到访客自己的语言，`Accept-Language` 说什么就去哪儿——
  ...
  **这张表 09-10 是拿别人那条量的**（`ublock-origin`——我们自己那条当时还是 404），
  09-15 上架之后拿 `doubak` 自己重量了一遍，四行分毫不差。留着这句是因为**在一个
  代理上量到的结果与在本体上量到的，不是同一种证据**，而这个项目为「代理」这件事
  已经付过八次学费；这一次代理是对的，那也得是量出来的，不是推出来的。

  写死 `/en-US/` 的话，这个项目**绝大多数读中文的用户会落在英文页上**——而
  `_locales` 里特意备了简体与繁體两份商店文案，正是为了他们。上架之后这一点也不用
  再推测了：AMO 的接口直接把三份都列出来，`name` 底下三把钥匙
  （`en-US` / `zh-CN` / `zh-TW`），所以 `_locales` 那条路是真的通到商店页上的。

  这与两个 Chromium 商店那条「只写扩展 ID、不带名字那一段」是同一条规矩：
  **别把一个会变的段写死**，区别只在那边变的是名字，这边变的是语言。
