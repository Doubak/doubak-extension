...
- **后台事件页能不能扛住几小时的抓取。** 这是最贵的一个，探针答不了，要真跑一次
  ...
  每页写检查点、可恢复的）。
- **zip 写好之后怎么交出去**：先写进 OPFS 再 `getFile()` → `createObjectURL()` →
  `<a download>`，还是直接攒成 `Blob`。两条路的内存表现都没量过（真实段有 159 MB），
  而「绝不在内存里拼一个巨大的 Blob」是标准规矩——所以**先量再选**，别照猜的写。
  这是导出那条路上最后一块。
...
`showDirectoryPicker` 就报错」。接上去之前要先答完上面那条「怎么交出去」。
