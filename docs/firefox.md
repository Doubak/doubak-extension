...
Firefox 上「导入档案…」仍然报「请使用 Chrome 或 Edge」——那一侧要换成
`<input type="file" webkitdirectory multiple>`（每个 `File` 带 `webkitRelativePath`，
整棵树一次拿全）。`test/destination.test.js` 里有一条测试**钉着这句话还在**，
所以做完那一步时会红，改的人必须顺手把导出这边的规则扩过去。
