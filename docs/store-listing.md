...
> **这一份只管 Chrome 应用商店。** AMO（Firefox）那边要填的东西
> ——扩展 id、联系邮箱、`strict_min_version`、`web-ext lint` 剩下那条警告
> ——在 [`firefox.md`](firefox.md)，**不在这儿抄一遍**：一份说明出现在两处就会分叉，
> 而分叉的方向是「其中一处还写着旧的 id」，那正好是那个字符串唯一不能出错的地方。
> 权限理由两边通用，下面那几段照抄即可。
...
> 标签、广播、日记与影评、自己编的豆列（连同你写在每个条目上的评语），
> 连同你自己上传的图片。
>
> 抓完就能直接看：档案页里可以把存下来的页面就地解析成条目——你的短评、广播、
> 豆列评语——用来确认抓到的确实是自己的东西。**不联网，也不写入任何东西。**
>
