 ...
 * ## 检查必须**两边都有**
 *
 * 这边的 `test/vendor.test.js` 一直在守，而且比上游那三条查得细。它没能拦住
 * 2026-09-04 那次，原因不是它写得不好，是**它长在改动作者到不了的仓库里**：
 *
 * ```
 ...
 * ```
 *
 * 接下来两天，命令行出的站点是修好的，而**面板里「导出 → Markdown」出的站点仍然
 * 带着那个 bug**。两个宿主对同一份档案给出不同结果，正是这套机制存在的全部理由。
 *
 * 所以那三个仓库各自也有一条 `test/vendor-freshness.test.js`，读的是**这里这份
 * 名单**（名单只有一份，抄第二份就会漂）。改上游的人跑的是上游的 `npm test`，
 * 那就得是那边红。
 *
 * 代价照说：两个仓库没有原子提交，所以**跨仓库的改动会红一会儿**——先推上游，
 * 上游红；再在这里跑一遍同步推上去，两边都绿。那段红是对的，上一次同样的不一致
 * 完全无声，持续了两天。
 *
 * ## 只搬纯函数
 *
