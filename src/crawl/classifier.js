 ...
 *
 * ## 「谁上传的」不够，还要问「它属于谁」（2026-09-06 收紧）
 *
 * 实测撞上一类两边都不是的东西：**传到作品相册里的照片**。17 条广播写着
 * 「MewX 上传了 N 张照片到《寂静之人》的相册」，一共 51 张，`data-uid` 是本人
 * ——按「谁上传的」这一条，它们该进 `assets-*`。
 *
 * **但它们不抓，这是明确的范围决定（2026-09-06，项目主人定）。** 传进作品相册
 * 的照片是**对这个条目的贡献**，跟着条目走而不是跟着账号走：所有人都看得见，
 * 保留与删除由条目那边决定，注销账号也不必然带走它。这正是 CLAUDE.md 里
 * 「用户创作内容与目录数据永远不放在一张表里——保留期、删除语义、法律位置都不同」
 * 那条规矩，所以它归目录那一侧。真要留，那是 enricher 之后再说的事，甚至不是 enricher。
 *
 * 一句话记法：**上传这个动作是你的，上传的东西属于条目。**
 * 那 17 条广播（你写的那句话、日期、指向哪个相册）照抓不误，照样进档案；
 * 不进档案的是那 51 张照片本身。
 *
