...
export class Exclusive {
   ...
   * ## 判死之后不再翻案
   *
   * 已经超过阈值的持有者，**再吭一声也不算数**——只有抢占能让它翻篇。
   *
   * 原来这里是无条件刷新的，实测出过事（8494 秒的那次）：一段推进卡死之后
   * 早就该被判死了，而用户按下「暂停」时 `runner.pause()` 发出的那条 `paused`
   * 事件走的是同一个事件通道，于是**替那具尸体刷新了心跳**。紧接着的「继续」
   * 被拒，理由是「已经有『抓取』在进行中（8494 秒前开始）」——一个刚刚才被
   * 别人证明「还活着」的死持有者。
   *
   * 两条道理，缺一不可：
   *
   * 1. **活着是连续的。** 静默超过阈值这件事已经发生了，一条迟来的消息改变不了
   *    它。判据是「有没有连续地吭声」，不是「最近一次吭声是什么时候」。
   * 2. **证明必须由持有者自己出具。** 而这个类看不出一条 `touch()` 是谁调的
   *    ——真正的分辨在调用侧（见 offscreen.js 的 `relayEvent`）。这里挡住的是
   *    误判之后**永远翻不了身**的那一半：不论谁来敲，死了就是死了。
   */
  touch() {
    if (this._held && !this.stale) this._held.beat = this._now();
  }
