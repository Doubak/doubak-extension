...
export class CrawlLoop {
   ...
   * 同理不在这里处理。
   *
   * 用户**自己上传**的图是另一回事：不可替代，该进 `assets-*`。现状：
   *
   * | 来源 | 状态 |
   * |---|---|
   * | 广播附图 | 已抓 —— `_enqueueStatusPhotos`，实测三种形态 |
   * | 相册 | 不需要 —— 实测个人主页写着「创建0」，这个账号没有相册 |
   * | **日记/评论正文内嵌** | **未支持**，见 routes.js 的 `UNRESOLVED_ROUTES` |
   *
   * 最后那条缺的是样本：手上两份真实正文页正文里一个 `<img>` 都没有，内嵌图的结构
   * 完全未知。而广播附图实测有三种形态同时存在——没有样本一种都猜不到。
   *
  ...
  _enqueueNextPage(item, route, profile, res, captureId, items) {
    ...
    //
    // **但这个计数器是按「路线」数的，而有的路线一条底下装着好几串互不相干的分页。**
    //
    // 豆列详情页就是：一条 `doulist.item` 路线底下有 6 份豆列，每份自己从 start=0
    // 翻到头。而 frontier 是**广度优先**的——实测这一趟的抓取顺序是六份豆列的第 1 页、
    // 再六份的第 2 页……于是「连续两页全空」会跨着不同的豆列凑齐：
    //
    //     162349128 空 · 54002196 24条 · 45473911 20条
    //     42193343 空 · 3913537 空 ← 连续第 2 个空页，判停滞
    //
    // 后果是 **3913537 因为「别的豆列翻到头了」而被停在第 2 页**，不再入队第 3 页。
    // 这一趟侥幸没丢东西（那几份本来就完了），但换成一份第 3 页还有内容的豆列，就是
    // 静默截断——而这条路线连声称数都没有，漏了没有任何信号。
    //
    // 所以：**有 `nextPageUrl` 的路线（每个条目自带一串分页）不看这个路线级计数器。**
    // 它们靠 `last_page`（页面自己说）与下面那条「本页一条都没有」收尾，判的都是
    // **这一串**的终点，不受别的串影响。
    if (progress.stalled && !route.nextPageUrl) {
      state.markFinished();
      this._emit({
        type: 'route_finished',
        routeKey: item.routeKey,
    ...
    // **每个条目自带一串分页的路线：本页一条都没有 = 这一串到头了。**
    //
    // 判的是「这一串」，不是整条路线——与上面那段说的是同一件事。放在这里而不是靠
    // 路线级的空页计数器，正是为了不让别的豆列的空页影响这一份。
    //
    // 用「本页一条都没有」而不是「不满一页」：后者已经被实测证伪（游戏列表第 7、14、
    // 17 页只渲染 14/14/13 条）。多页豆列另有 `last_page` 兜着，走到这一步的基本都是
    // 单页豆列的第 2 页。
    if (route.nextPageUrl && items.ids.length === 0) {
      state.markFinished();
