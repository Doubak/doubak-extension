...
const POLICY = {
   ...
   * ## 为什么要单独一条
   *
   * 没有网络时，每一个请求都要**熬满 30 秒超时**才算失败，一批 25 条就是十几
   * 分钟——而预算（22 秒）只在批与批之间检查，拦不住进行中的那一批。实测撞到过：
   * 电脑闲置、网络断开，一段推进跑了 **780 秒**，期间心跳每 30 秒来一次、每次都
   * 只能说「上一段还在跑」。用户看到的是二十来行「未恢复」，像是彻底卡死了。
   *
   * 判成一个明确的原因，就能**早早停下**并把这十几分钟省掉。
   *
   * ## 自动恢复，而且要安静
   *
   * 网络恢复不需要用户做任何事，所以 `autoResume: true`；也不该弹通知——
   * 「你的网断了」这种事用户比我们清楚，为它响一声只会训练他忽略通知。
   *
   * 与 `crash` 的区别在于它是**有信号的**：我们知道为什么停，也知道该等什么。
   */
  network_down: {
    autoResume: true,
    reason: '网络连不上豆瓣，等网络恢复后自动继续',
    userVisible: false,
  },
   ...
   * 驱动层空转：连着几批什么都没推进。
   *
   * **绝不自动恢复。** 这是插件自己的 bug，不是豆瓣那边的状况——自动重试只会
