...
async function abortCrawl(bundleId) {
  ...
  const lines = [
    ...
    '',
    // **最后一句原来是一句「中止之后就可以把它删掉」。**（原文不在这儿重打一遍：
    // `test/ui.test.js` 反着钉那句话不许再出现，注释里抄一遍会把测试自己染红。）
    //
    // 它说的是锁：runner 放手之后
    // `deletable` 才翻成 true。但它出现在确认框的最末一行，也就是最被记住的位置，
    // 于是读起来是一句**建议**——而这恰恰是用户最容易把「这次没抓完」误当成
    // 「这份没用」的时刻。它还跟三行之上的「将全部保留」自相矛盾。
    //
    // 能力照说，但降级成从句，并且把最后一句留给这份档案还能做什么。
    '中止后该档案将解除占用，届时可以删除——但通常不必：',
    '半途中止的档案照样能解析、能导出，导出之后也能再导入回来。',
  ];
