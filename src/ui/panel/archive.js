...
/**
 * 这一对小标签现在开着哪个：`'captures'`（默认）/ `'content'` / `null`（都收着）。
 *
 * ## 默认开着「翻看捕获」
 *
 * 原来默认两块都收着，理由是这一页已经装着导入、导出、校验、删除、用量，再摊开
 * 一大块会没法看。但**标签这个样子本身在说「下面有内容」**，而收起来时下面什么都
 * 没有，于是它更像两个没人按的按钮——报上来的原话是「不明显」。
 *
 * 挑「翻看捕获」而不是「查看内容」，是因为它**不花钱**：index 早就读进来了，画的是
 * 已有数据；而「查看内容」一展开就要取记录、解压、抽取，默认打开等于点一下档案就
 * 白干一次活（见 content.js）。
 *
 * **跨档案保留**：逐条核对字节的人一定会连着看好几份，每换一份都收回去，就等于每换
 * 一份都要再点一次。模块级变量而不是每次渲染重算——它是用户的选择，不是从数据推出来
 * 的状态。「都不选」照旧是合法状态：再点一下当前那个就收起来。
 */
const DEFAULT_PANE = 'captures';
let openPane = DEFAULT_PANE;
...
function renderBundlePicker(items) {
  ...
  // **只有一份时也要画。**
  //
  // 原来的判据是「≤1 份就不画」——理由是「没什么可选的」。那句话只说对了它的一半
  // 用途：它不只是个选择器，还是**这一栏在讲哪一份档案**的唯一交代。只有一份时
  // 把它收掉，右边那些摘要、导出、校验按钮就没有了主语；而抓出第二份的那一刻，
  // 整页版式又会突然变样。
  //
  // 真实反馈两次都是同一句：「左边的档案清单看不见」。第一次我判成「≤1 份，本来
  // 就不该画」，只把那条空栏收干净了——**那是把用户报的现象当成了正确行为**，
  // 而他报的正是那个现象本身。
  //
  // 一份都没有时才收：那时右边已经写着「还没有档案。开始一次抓取之后这里会显示
  // 内容。」，左边再空一栏只是噪音。收掉那一列靠 CSS
  // （`.with-aside:has(> aside[hidden])`），版面是主题的事。
  ...
  try {
    ...
    if (!s.hasManifest) {
      ...
      note.append(b, document.createTextNode(
        ...
        // **把「不是坏的」补成「能用来做什么」。** 前半句只否定了一件事，而看到
        // 一屏空字段的人接着要问的是「那它现在有什么用」——不答的话，答案由他自己猜。
        + '即便这次抓取就此中止，这一份也不会作废：解析器读的是索引与页面，'
        + '导出的目录以后也可以再导入回来。',
      ));
