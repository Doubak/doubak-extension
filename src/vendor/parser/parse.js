...
// 0.13.0：**广播多出 visibility**。抓取跑在用户自己的登录态下，所以时间线上包含
//        **只有他自己看得见**的条目，而此前 canonical 里它们与公开广播无从分辨。
//        实测后果：一篇私密日记的全文出现在样张站的首页上——豆瓣发日记时会同步
//        一条广播，正文一字不差。判据钉在**内层** `div.status-item` 的 class 上
//        （`status-item private`）；外层那个 wrapper 上没有这个词，所以不能在整段
//        里找 `private`。实测 1112 张时间线页、21996 个块：private 恰好 1 个，
//        deleted 7 个。日记那一侧 0.12.0 就补了 visibility，广播这一侧漏了——
//        **同一条规则，在隔壁那个记录类型上缺席**，这个文件里已经是第三次。
// 0.11.0：广播多出 action_parts —— 动作句里那几个链接（收藏到哪个豆列、传照片到哪个
//        相册）。此前只留了文字：`收藏游戏到豆列 游戏购买小账本` 在站点上是一句点不动
//        的话，而它在豆瓣上三个词都能点。不变量是 parts 拼起来逐字等于 action。
// 0.10.0：**广播的动作取整句，不再是「前七个字」**。那个 7 看起来是最长的动作词
//        （`收藏图书到豆列` 正好七个字），实际是「在第一个行内链接前刹住」——而链接里
//        装的正是宾语。实测 61 条广播因此一直没记下收藏到了哪个豆列，另有 33 条广播
...
function upsertBroadcast(store, { b, account, observation, parserVersion }) {
  const fields = {
    ...
    text: b.text,
    action: b.action,
    // 动作句里那几个链接（豆列名、作品名、相册…）。**拼起来逐字等于 `action`**，
    // 所以下游重建带链接的句子不需要任何子串匹配——而子串匹配对不上时是静默的。
    // 没有链接的广播这里是 null：3423 条里带链接的只有 90 条。
    action_parts: b.actionParts,
    // **只有作者本人看得见的那些广播。** 抓取跑在用户自己的登录态下，所以时间线
    // 上包含这类条目；不记下来，下游就分不出它们与公开广播，而站点默认照发——
    // 实测这正是一篇私密日记的全文出现在样张站首页上的原因。日记那一侧 09-07 就
    // 补了 `visibility`，广播这一侧漏了。判据钉在 `div.status-item` 的 class 上，
    // 认不出容器时是 `null` 而不是 `public`（同又名的 null / []）。
