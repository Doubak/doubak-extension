...
describe('面板脚本', () => {
  ...
  test('帮助页要回答「我该点哪个」', async () => {
    ...
    // **重试预算放宽到 11 次之后，「正在退避」与「卡死了」在日志里长得一模一样。**
    // 而两者的处置完全相反：后者要去查，前者只要等。这句话必须在帮助里查得到，
    // 否则用户唯一的判据就是「同一行 URL 出现了很多次」——那正好指向错误的结论。
    assert.match(help, /最多 11 次/, '帮助里没写重试几次');
    assert.match(help, /固定等 10 秒/, '帮助里没写两次之间等多久');
    assert.match(help, /不是卡住/, '帮助里没说清「反复出现」不等于卡死');
