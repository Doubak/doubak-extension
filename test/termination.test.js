...
describe('一条路线底下装着好几串分页时，各串互不影响', () => {
  test('**别的串翻到头了，不许把这一串也停掉**', () => {
    // 实测这一趟（bundle 20260814T223824Z-4b82f3）：frontier 是广度优先的，抓取顺序
    // 是六份豆列的第 1 页、再六份的第 2 页……于是「连续两页全空」跨着不同的豆列凑齐：
    //
    //   162349128 空 · 54002196 24条 · 45473911 20条
    //   42193343 空 · 3913537 空 ← 连续第 2 个空页，判停滞
    //
    // 3913537 因此被停在第 2 页、不再入队第 3 页——**它是被别的豆列的空页停掉的**。
    // 那一趟侥幸没丢东西（那几份本来就完了），但换成一份第 3 页还有内容的豆列就是
    // 静默截断，而这条路线连声称数都没有，漏了没有任何信号。
    assert.match(
      nextPageFn, /progress\.stalled && !route\.nextPageUrl/,
      '每个条目自带一串分页的路线，不该看路线级的停滞计数器',
    );
  });

  test('这类路线靠「本页一条都没有」收自己这一串', () => {
